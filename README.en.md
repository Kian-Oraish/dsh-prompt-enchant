...
3. **Design-mode professional layer** (v0.5.1 universal formula): for AI image creation (text-to-image / image-to-image / interactive edit) and video creation (text-to-video / image-to-video / first-last frames), organized by the universal formula "classify → anchor → translate → fill slots → shape" — dual slot sets for image and video, storyboard timeline (shot durations must sum to the total), first-to-last-frame transition, colloquial-word dictionary and channel-aspect-ratio dictionary; unconfirmed elements use the strict format 【待确认:element(candidates)】 and can be completed right in the popup; non-creative input automatically falls back to general enhancement.
...
8. **【待确认】popup confirmation** (v0.5.0): missing elements the model cannot reliably infer (`【待确认:element(candidates)】`, with or without brackets) automatically trigger the official DSH question card — pick candidates or type a custom answer per question; on submit, answers are substituted back and the text is re-enhanced and filled in, leaving no 【待确认】 residue; a 90-second no-answer/unavailable fallback degrades to plain text marks, never blocking.
9. **Universal-formula design layer** (v0.5.1): the design layer was rebuilt from two flat lists into one universal formula + dual slot sets + six sub-mode cards + two dictionaries + an output skeleton; 【待确认】 uses a strict format (candidates split by "/", no commas/slashes/parentheses inside candidates, "+" for parallel terms), with the four anchor section titles and General-mode behavior unchanged; the paste-form `dynamic/` mirror is frozen from this version, and the prompt truth sources shrink to two: `lib/modes.js` + `config/enhance-prompt.md`.
...
**【待确认】popup confirmation** (v0.5.0): rides the official `ctx.userQuestions` seam — the client passes the current `sessionId` through the slot inject, the host validates it as a live root agent and the official card takes over that session's composer; answers flow back on the same promise, are substituted into the original text, and the pipeline re-runs (inner run suppresses re-popping to prevent loops); binding failure / timeout / cancel / no client all degrade to plain-text marks (same behavior as v0.4.0). The self-test tool suppresses the popup by default. See `docs/测试案例.md` for acceptance test cases.

**Universal-formula design layer** (v0.5.1): `DESIGN_LAYER` is upgraded to the "classify → anchor → translate → fill slots → shape" universal formula, with image/video slot sets, a storyboard timeline, six sub-mode cards, a colloquial-word dictionary and a channel-aspect-ratio dictionary; 【输出格式】 locks the strict confirmation format and the candidate character-set rules ("/" between candidates, no commas/slashes/parentheses inside a candidate, "+" for parallel terms) so popup candidates always parse. Self-test design cases grew to 7 (six sub-modes + question fallback); the new `tests/design-layer.test.mjs` locks keywords and asserts the two truth sources match verbatim; the `dynamic/` paste mirror is frozen (keeps the old layer for reference only). See the 0.5.1 section of `docs/测试案例.md` for acceptance cases.
...
**Cost gates** (v0.7.0): a single enhancement can trigger up to 4 model calls (first call + empty-body retry + validation retry + post-dialog rerun). Therefore model calls are counted independently (`MAX_MODEL_INFLIGHT=2`), the **HTTP slot is released while the confirmation dialog waits** (the old 90-second hold could 429 a third real request; now 45 seconds), the 45-second timeout goes through the framework's `deadline()` and **actually cancels the old stream**, closing the tab / switching sessions aborts the in-flight enhancement instead of idling, and a user cancellation is **never retried**. The agent-visible `prompt_enhance_selftest` tool is **off by default** (it runs 12 real model calls); enable it with `config: { debugTools: true }` on the composition line.

> **Which stream to listen on for "abort on client disconnect" (v0.7.1 errata):** it must be the **`'close'` event of `res`**, never of `req`. Node emits `req`'s `'close'` as soon as the **request body has been read** — not when the connection drops — so listening on it made **every healthy request** abort itself the moment its body finished; and because the abort path then concludes "the client is gone" it **writes no response at all**, leaving the client stuck in a permanent loading state. `res`'s `'close'` fires exactly once, on response completion or connection teardown, at which point `res.writableEnded` reliably distinguishes the two. Stub doubles cannot reproduce this timing, so the regression tests run against a **real `node:http` server**.
