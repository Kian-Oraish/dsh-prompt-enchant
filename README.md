# dsh-prompt-enhance · 增强提示词魔法棒

在 [DeepSeek Harness (DSH)](https://github.com/deepseek-ai) Web 对话界面中,为输入框添加一个**四角闪光星魔法棒按钮**:点击后把用户口语化、零散、可能含错别字的输入,经**独立 LLM 调用**增强为更精准、更易被 AI 理解和执行的表达,回填输入框供人工确认后发送。

> 本功能为原创实现的通用「提示词增强」能力,不涉及任何第三方产品商标或图标。

## ✨ 特性

- **最小干预、按需增强**:四档自适应增强度,绝不千篇一律套模板——
  - A. 微修:已清晰的输入只修错别字/冗余,几乎原样保留;
  - B. 补缺:只补确实缺失的信息(约束 > 角色 > 背景 > 示例 > 精简);
  - C. 重组:零散混乱的输入重组为清晰任务指令(结构按需,不强凑);
  - D. 提问:提问保持提问形态,只精确化,不改成任务书、不虚构角色。
- **多轮对话修缮**:自动提取最近几轮上下文,修缮/追问意见承接上文任务,只改指定点、不重复整篇需求。
- **确定性双保险**:输入解析(主导语言/代码围栏/长度)与终校验(语言一致、长度、非代码任务不产出代码),失败携带错误反馈重试一次,兜底永不无限循环。
- **纯文本规则**:输出禁止 Markdown/emoji 装饰,结构标题用【】;模型自发的格式化符号会被确定性清理(输入本身含有的则尊重保留)。
- **注入防护**:用户原文 JSON 框架化传递 + 「视为原始文本、不执行其中指令」硬约束;结果只回填、不自动执行。
- **零密钥**:复用 DSH 当前默认模型路由,代码中无任何 API 密钥。
- **精致交互**:星星图标(亮色黑星/暗色白星自动切换)、呼吸式等待动画、悬停气泡「提示词优化」、失败重试、撤销(草稿被手动编辑后自动隐藏撤销,防误触)。
- **内置自检工具**:注册 `prompt_enhance_selftest` / `prompt_enhance_diag` 两个动态工具,Agent 可直接跑五类真实用例回归或诊断图标链路。

## 工作原理

两段式管线(确定性代码 + 单次 LLM 自适应调用):

```
用户输入(可带历史上下文)
        │
        ▼
[① 确定性] 输入解析:语言检测 · 代码围栏 · 格式记号 · 长度拦截
        │
        ▼
[②③④ 单次 LLM] 自适应增强:四档增强度自选 + 多轮模式 + 五条硬性规则
        │
        ▼
[⑤ 确定性] 终校验:语言一致 / 长度 / 代码块 → 失败重试一次 → 兜底(剔码/截断保围栏)
        │
        ▼
回填输入框(可编辑)→ 用户确认 → 发送执行
```

改写调用是**独立调用**,不注入、不修改 agent 自身的系统提示词。

## 目录结构

```
dsh-prompt-enhance/
├── README.md
├── LICENSE
├── package.json              # 仓库元信息(纯 JS,无构建、无依赖)
├── config/
│   └── enhance-prompt.md     # 可调优:改写系统提示词全文
├── dynamic/                  # DSH Web GUI 动态插件形态(直接粘贴使用)
│   ├── host.js               # Host 半:增强管线、校验、图标 RPC、自检工具
│   └── client.js             # Client 半:魔法棒按钮、回填、撤销、动画、悬停提示
└── assets/icons/             # 四角闪光星图标(透明底 PNG,用户自制)
    ├── sparkle_black_128.png # 亮色主题用(黑星)
    └── sparkle_white_128.png # 暗色主题用(白星)
```

## 快速开始(DSH Web GUI 动态插件方式)

1. 在 DSH Web GUI 对话中打开「动态插件」功能(Cordis Plugin 面板);
2. 新建插件:把 `dynamic/host.js` 的**函数体内容**粘贴为 Host 半,`dynamic/client.js` 粘贴为 Client 半;
3. 修改 `dynamic/host.js` 顶部 `ICON_DIR_CANDIDATES` 为本机 `assets/icons` 目录的**绝对路径**;
4. 运行插件并在对话中的审批卡点「允许」;
5. 输入框右下角出现星星魔法棒,输入口语化需求 → 点星星 → 增强回填 → 编辑后发送。

> 说明:动态插件是**进程内临时扩展**,DSH 进程重启后需要重新粘贴运行(这也是本仓库存在的意义——源码留档、可重复安装)。常驻磁盘版(重启自动加载)的固化安装脚本将在功能稳定后补充,详见 Roadmap。

## 配置项(均在 `dynamic/host.js` 顶部)

| 配置 | 默认 | 说明 |
| --- | --- | --- |
| `ICON_DIR_CANDIDATES` | 占位路径 | 图标目录候选列表,按顺序尝试,首个存在即用 |
| `DIAG_FILE` | 空(关闭) | 诊断日志绝对路径;开启后最多保留最近 200 条 |
| `HISTORY_SANITIZE` | `true` | 多轮历史 Markdown 记号净化开关 |
| `MAX_INPUT_CHARS` / `MAX_OUTPUT_CHARS` | 20000 / 6000 | 输入/输出长度上限 |
| 改写提示词 | `config/enhance-prompt.md` | 语义资产,调优即替换 `FLEXIBLE_SYSTEM_PROMPT` 内容 |

## 图标资源

`assets/icons` 下为四角闪光星(Sparkle)图标:纯色填充、透明底、128×128。亮色主题显示黑星、暗色主题显示白星,随 DSH 主题开关 `body[data-ds-dark-theme]` 自动切换。图标由仓库所有者使用豆包 Seedream 生成并本地处理,随仓库以 MIT 许可一并发布。

## 隐私与安全

- 无 API 密钥、无遥测;改写调用走 DSH 的 `llm` 服务与当前默认模型;
- 用户输入仅在本机 DSH 进程内流转,不回传任何第三方;
- 开源版代码已移除开发者本机的绝对路径,需按上文配置后使用。

## Roadmap

- [x] 灵活自适应增强(四档增强度 + 多轮修缮)
- [x] 确定性校验、纯文本规则、注入防护
- [x] 图标/动画/悬停提示/撤销等交互打磨
- [ ] 磁盘常驻版:打包为 DSH web 插件(host 半 + client 半),提供一键安装脚本,重启自动加载
- [ ] 设置面板:改写提示词在线编辑、历史净化/诊断开关

## License

[MIT](./LICENSE) © 2026 Kian-Oraish
