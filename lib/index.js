...
import { buildSystemPrompt, hasMode, defaultMode, publicModes } from './modes.js'
...
export function apply(ctx, config) {
  ...
  async function enhance(rawText, rawHistory) {
    ...
    // 模式建议钩子(设计树已定):响应固定携带 suggestedMode 字段。
    // 单模式时代恒为 null(零行为变化);多模式迭代时由增强调用附带
    // 模型判定的建议模式 id(仅当建议有效且 ≠ 当前模式)。
