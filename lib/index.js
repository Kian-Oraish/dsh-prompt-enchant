...
// schemastery(设置 schema)按需加载:解析失败不影响插件加载,降级为手写 schema
let schemaLib = null
try {
  schemaLib = await import('@deepseek-ai/schemastery')
} catch (err) {
  console.error('dsh-prompt-enhance: 加载 schemastery 失败,设置 schema 将使用内置降级实现:', err)
}
...
export function apply(ctx, config) {
  ...
  // settings.yaml,DSH 设置后台由客户端半渲染一级栏目。旧框架无该服务
  // (或注册失败)时:模式恒为「通用」,增强管线行为不受影响。
  // 注意:设置服务由 dsh-settings-file 提供,其提供时机可能晚于本插件
  // apply(应用顺序竞态,已在 0.1.2-alpha.5 实测复现)——故采用惰性获取:
  // apply 时尝试一次,未拿到则监听 'service' 事件补挂,currentMode 每次
  // 使用前也会再尝试。
  ...
  if (!settingsRegistered && !settingsFailed) {
    // 应用顺序竞态兜底:设置服务晚于本插件提供时,由服务事件触发补挂
    try {
      ctx.on('service', () => { ensureSettings() })
    } catch (err) {
      console.error('dsh-prompt-enhance: 监听 service 事件失败:', err)
    }
  }
  ...
  // 模式 schema:优先 schemastery;解析失败时用手写「函数 + toJSON」实现
  // (满足 ctx.settings 的 resolve/toJSON 契约,设置后台通用解码可复用)
  function modeSchema() {
    if (schemaLib !== null) {
      const z = schemaLib.default !== undefined ? schemaLib.default : schemaLib
      try {
        if (typeof z.object === 'function' && typeof z.string === 'function') {
          return z.object({ mode: z.string() })
        }
      } catch (err) {
        console.error('dsh-prompt-enhance: 构造 schemastery schema 失败,使用降级实现:', err)
      }
    }
    const fn = (value) => {
    ...
    fn.toJSON = () => ({
      type: 'object',
      properties: { mode: { type: 'string', description: '增强模式 id,当前可选: ' + publicModes().map((m) => m.id).join(', ') } },
      required: ['mode'],
    })
    return fn
  }
  ...
  async function confirmEnhance(text, rawHistory, sessionId) {
    ...
    ctx.timeout(CONFIRM_TIMEOUT_MS).then(() => { try { controller.abort() } catch (err) { /* 已结算,忽略 */ } })
    try {
  ...
  const selftestTool = defineToolSafe({
    ...
    description: '增强提示词插件自检:用真实用例跑完整增强管线,返回输出与校验结果。可指定 case(vague/question/wellformed/code/design_image/design_video/multiturn)或直接传 text 跑任意输入。',
    parameters: {
