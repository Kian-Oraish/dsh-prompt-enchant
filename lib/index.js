...
// 版本号的**单一事实源**是 package.json —— 不再在诊断工具里硬编码字符串。
// 这个项目反复被「声明与现实脱节」咬过(幽灵字段 dsh.engines、失效的 semver
// 区间、node_modules 里钉死的 devDeps),硬编码版本号是同一类债:改 package.json
// 忘了改这里,诊断工具就会自信地报一个过期版本。读不到时如实报 'unknown'。
const PKG_VERSION = (() => {
  try {
...
// ---- 插件 Config:增强模式(v0.7.2 适配 rc.2 的 settings 契约) ----
// DSH 0.1.7-rc.2 改写了 settings:不再有 ctx.settings.register(ns, schema, {base, validate}),
// 改为「设置表单以 **profile 条目 id** 标识插件,只投影该条目 Config 的 `.volatile()` 字段,
// 写入经当前 profile 的 Cordis patch 持久化」(见 @deepseek-ai/dsh-settings README;
// 框架自带的 dsh-agent-default-model 就是 `z.string().required().volatile()` 这个写法)。
// 旧框架(0.1.6 ~ 0.1.7-alpha.x)仍走命名空间注册,见 apply 内保留的兜底路径。
//
// `.volatile()` 是 schemastery **3.18.4** 起才有的 API(3.18.2 无此方法),而插件可被
// 更旧的框架加载、开发态也可能解析到旧副本,故按能力探测降级:没有 `.volatile()` 时
// 退化为普通字段(那条路径本来就用不上投影,读值走旧的 settings 命名空间)。
const MODE_FIELD = z.string().description('增强模式 id,当前可选: ' + publicModes().map((m) => m.id).join(', '))
export const Config = z.object({
  mode: typeof MODE_FIELD.volatile === 'function' ? MODE_FIELD.volatile() : MODE_FIELD,
})

export function apply(ctx, config) {
  /** 读一个配置项:rc.2 的 Config 是**反应式引用**(`{ get() }`),旧框架是普通值,两种都要能读
   *  —— 旧代码用 `config?.x` 直读,在 rc.2 下拿到的是引用对象,选项会被静默忽略。 */
  const cfg = (key, fallback) => {
    let value
    try {
      value = config === null || typeof config !== 'object' ? undefined : config[key]
      if (value !== null && value !== undefined && typeof value === 'object' && typeof value.get === 'function') value = value.get()
    } catch (err) { value = undefined }
    return value === undefined ? fallback : value
  }
  const opts = {
    diagFile: typeof cfg('diagFile', '') === 'string' ? cfg('diagFile', '') : '',
    maxInputChars: Number.isFinite(cfg('maxInputChars', 20000)) ? cfg('maxInputChars', 20000) : 20000,
    maxOutputChars: Number.isFinite(cfg('maxOutputChars', 6000)) ? cfg('maxOutputChars', 6000) : 6000,
    historySanitize: cfg('historySanitize', true) !== false,
    temperature: Number.isFinite(cfg('temperature', 0.3)) ? cfg('temperature', 0.3) : 0.3,
    // 是否「显式」配置了 temperature:未显式配置时,推理档请求不再盲发
    temperatureExplicit: Number.isFinite(cfg('temperature', undefined)),
    // Agent 可见的调试工具(selftest):默认关闭。一跑就是 12 个真实模型调用,
    // 任何 agent 都能触发,且成本与耗时都很高 —— 故默认只注册诊断工具(diag,零成本)。
    debugTools: cfg('debugTools', false) === true,
  }
  ...
  // ==================== 设置:增强模式 ====================
  // 模式值的来源按框架版本自动选择:
  //   · rc.2+ :上面导出的 Config 的 volatile 字段(设置表单投影它,写入落 profile patch)
  //   · 旧版  :ctx.settings.register 命名空间「prompt-enhance」(落 settings.yaml)
  // 两条路径都保留、互不干扰;都不可用时恒为「通用」,插件其余能力不受影响。
  let modeScope = undefined
  let settingsRegistered = false
  let settingsFailed = false
  let settingsPolicy = false
  const configBound = config !== null && typeof config === 'object' && config.mode !== undefined
  ...
  function doRegisterSettings(settingsService) {
    ...
    try {
      ...
      appendDiag({ stage: 'settings-register', ok: true, detail: '命名空间 prompt-enhance 已注册(旧契约)' })
    } catch (err) {
      settingsFailed = true
  ...
  // rc.2 契约:自带设置栏目的插件在 settings 子级里以 effect 注册策略
  // configure({ auto: false }, ctx.fiber) —— 声明「本插件自己渲染设置,不必按 schema 生成表单」。
  // 框架自带的 dsh-agent-default-model 即此惯用法;该策略不移除配置读写。
  function doRegisterPolicy(child) {
    if (settingsPolicy) return
    try {
      if (child === undefined || child.settings === undefined || typeof child.settings.configure !== 'function') return
      if (typeof child.effect !== 'function') return
      child.effect(() => child.settings.configure({ auto: false }, ctx.fiber))
      settingsPolicy = true
      appendDiag({ stage: 'settings-policy', ok: true, detail: 'configure({auto:false}) 已注册(自带设置栏目)' })
    } catch (err) {
      appendDiag({ stage: 'settings-policy', ok: false, detail: String(err && err.message !== undefined ? err.message : err) })
    }
  }
  ...
  try {
    ctx.inject(['settings'], (settingsCtx) => {
      doRegisterSettings(settingsCtx.settings)
      doRegisterPolicy(settingsCtx)
    })
  } catch (err) {
    // 极端情况下 ctx.inject 不可用(更老的框架):退回 apply 时尝试一次
    doRegisterSettings(ctx.get('settings'))
  }
  // 当前生效模式:rc.2 先读 Config 引用(反应式,改设置即刻生效),旧框架再读命名空间;
  // 值无效或两侧都缺席时回退「通用」。
  function currentMode() {
    if (configBound) {
      try {
        const raw = config.mode
        const live = raw !== null && typeof raw === 'object' && typeof raw.get === 'function' ? raw.get() : raw
        if (hasMode(live)) return live
      } catch (err) { /* 落到旧路径 */ }
      return defaultMode()
    }
  ...
  // 模式 schema(仅旧契约路径使用):纯 schemastery(见文件头 import 的说明)。
  function modeSchema() {
    return z.object({
  ...
  function errorPayload(err) {
    ...
    if (raw.includes('增强已取消') || raw.includes('PROMPT_ENHANCE_ABORTED')) return { code: 'CANCELLED', error: '增强已取消', status: 499 }
  ...
  registerRoute({
    ...
    handler: async (req, res) => {
      ...
      httpInflight++
      // ==================== 「客户端断开即 abort」的正确写法(v0.7.0 修正) ====================
      // 这个坑踩了两次,记录完整过程,别再踩第三次:
      //
      // ① 第一版写成 `req.on('close', abort)` —— 错。Node 在**请求体读完时**
      //    就会触发 req 的 'close',不是等连接关闭。于是**每一个健康请求**都在
      //    读完 body 的瞬间把自己 abort 掉 —— 用户看到「点了按钮立刻变红」。
      //
      // ② 第二版加 `if (res.writableEnded) return` 守卫,仍错:req 的 'close'
      //    触发得**比 body 读完还早**(真实 server 实测先于 readBody 完成),
      //    那一刻 writableEnded 必然是 false,守卫来不及生效 —— 结果更糟:
      //    请求被 abort 却又不回写响应(以为客户端已走),客户端**永久挂住**。
      //
      // ③ 正解:监听 **res 的 'close'**。它在「响应结束」或「连接断开」时恰好
      //    触发一次,且到那时 writableEnded 已能可靠区分两种情况:
      //      健康请求 → body(16) → 有结果 → 已写响应 → HTTP 200
      //      中途断开 → body(14) → res:close(writableEnded=false) → 中止
      const controller = new AbortController()
      const onResClose = () => {
        if (res.writableEnded === true) return // 正常写完响应,不是断开
        try { controller.abort() } catch (err) { /* 已结算 */ }
      }
      res.on('close', onResClose)
      const abortOnDisconnect = new Promise((_, reject) => {
        controller.signal.addEventListener('abort', () => {
          reject(new Error('增强已取消')) // errorPayload 映射为 CANCELLED
        }, { once: true })
      })
      abortOnDisconnect.catch(() => {}) // 防未处理 rejection
      try {
        ...
        const result = await Promise.race([
          enhance(args.text, args.history, { sessionId, signal: controller.signal }),
          abortOnDisconnect,
        ])
        jsonReply(res, 200, result)
      } catch (err) {
        // 客户端已断开:没人接这个响应,不必回写(写了只会污染错误日志)
        if (res.writableEnded === false && controller.signal.aborted === true) {
          appendDiag({ stage: 'enhance', ok: false, code: 'CLIENT_DISCONNECTED', detail: '客户端断开,已中止' })
        } else {
  ...
  const diagTool = defineToolSafe({
    ...
    execute: async (args, exec) => ({
      ...
      settings: {
        ...
        // rc.2 契约:插件 Config 的 volatile 字段 + configure({auto:false}) 策略
        configBound: configBound,
        policy: settingsPolicy,
        // 旧契约(0.1.6 ~ 0.1.7-alpha.x):settings 命名空间注册
  ...
  appendDiag({ stage: 'host-apply', ok: true, detail: `磁盘常驻版已激活, 模式: ${currentMode()}, settings: ${configBound ? (settingsPolicy ? 'config+policy' : 'config') : (settingsRegistered ? 'namespace(legacy)' : 'unavailable')}` })
}
