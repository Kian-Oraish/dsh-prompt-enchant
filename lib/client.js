...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      // 错误色令牌:核实结论 —— --dsw-alias-label-error **仍然是有效的主题令牌**
      // (框架内确实有定义),因此保留它,不做无谓改名。
      // 注:曾误判它「已不存在」并改成 state-error-primary,已回退 —— 那条结论
      // 来自一次不完整的 grep。这里同时给出 state-error-primary 作兜底链,
      // 两个令牌任一存在都能取到主题色,最后才落到硬编码。
      ...
      if (!mounted) {
        ...
        try {
          if (typeof ctx.inject === 'function') {
            ctx.inject(['slots', 'settingsScope'], () => { if (!mounted && !disposed) tryMount() })
          }
        } catch (err) { /* ignore */ }
      }
      ctx.effect(() => () => {
