...
// ----------------------------------------------------------------------------
// 由 DSH clientModules 伺服。URL 是**带内容哈希的 combo 形式**:
//   /plugins/??dsh-prompt-enhance/client.js&rev=<内容哈希>
// 因此 bundle 在服务端内存里按 rev 缓存 —— **改了 lib/client.js 必须重启
// DSH 守护才会发新字节**,浏览器刷新不够。
// 通过组合行挂载后,输入框工具行出现星星魔法棒按钮:
//   - 图标:内联 SVG(currentColor 单文件双主题)。v0.7.0 起不再请求任何
//     Host 图标路由 —— 那两条 /prompt-enhance/icons/*.png 已删除(死代码,
//     且是当时唯一绕过鉴权栅栏的路径)。
//   - 增强:fetch POST /prompt-enhance/api/enhance(带进程令牌,见下)
...
// v0.7.0 兼容修复:
//   · 引用保护闸门修好(旧版版本嗅探恒为 false → 闸门一直是关着的)
//   · 多轮上下文移交 Host(sessions.deriveMessages());删除已死的客户端历史链
//   · 进程令牌往返;在途请求随卸载/切会话取消;死兜底 ctx.on('service') 删除
// ============================================================================
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      // 错误色令牌(v0.7.2 复核):0.1.7-rc.2 的 dsh-client-ui-theme 只定义
      // --dsw-alias-state-error-primary / -secondary,**--dsw-alias-label-error 已被移除**。
      // 这里保持链式兜底,首选写 label-error 是为兼容旧框架;rc.2 会自动落到第二跳,
      // 两个令牌任一存在都能取到主题色,最后才落到硬编码红。
      '.pwe-statusbar b.pwe-status-error { color: var(--dsw-alias-label-error, var(--dsw-alias-state-error-primary, #e5484d)); }',
    ...
    // ==================== 进程令牌(v0.7.0,纵深防御) ====================
    // Host 每进程随机生成一个令牌;本客户端经带栅栏的 GET /prompt-enhance/api/token
    // 取一次,**只存内存**(绝不进 localStorage/sessionStorage —— 那会把纵深防御
    // 降级成另一个可被 XSS 读取的持久凭据),此后每个 API 请求带上去。
    // 令牌随 DSH 重启轮换;取不到时给明确可操作的报错,而不是静默失败。
    var tokenState = { value: null, inflight: null }

    function fetchToken() {
      if (tokenState.value !== null) return Promise.resolve(tokenState.value)
      if (tokenState.inflight !== null) return tokenState.inflight
      tokenState.inflight = fetch('/prompt-enhance/api/token', {
        method: 'GET',
        headers: { Accept: 'application/json' },
        credentials: 'same-origin',
      }).then((res) => {
        if (!res.ok) throw new Error('令牌请求失败: HTTP ' + res.status)
        return res.json()
      }).then((payload) => {
        if (payload === null || payload === undefined || typeof payload.token !== 'string' || payload.token.length === 0) {
          throw new Error('令牌响应格式异常')
        }
        tokenState.value = payload.token
        return payload.token
      }).finally(() => { tokenState.inflight = null })
      return tokenState.inflight
    }

    /** 丢弃缓存(仅用于诊断重取;令牌是每进程的,页面内重取结果相同)。 */
    function resetToken() { tokenState.value = null }

    /**
     * 调增强端点。v0.7.0 变化:
     *   · 不再传 history —— 历史由 Host 用 sessions.deriveMessages() 派生
     *   · 带 X-Prompt-Enhance-Token(进程级第二道闸)
     *   · 接受外部 signal(组件卸载 / 会话切换即取消,不再把结果写进错误的会话)
     */
    async function enhanceRpc(text, sessionId, externalSignal) {
      const controller = typeof AbortController !== 'undefined' ? new AbortController() : undefined
      const timeoutId = setTimeout(() => { if (controller !== undefined) controller.abort() }, 150000)
      // 外部取消与 150s 超时合流(AbortSignal.any 不可用时退回超时信号)
      var signal = controller !== undefined ? controller.signal : undefined
      if (externalSignal !== undefined && externalSignal !== null && typeof AbortSignal !== 'undefined' && typeof AbortSignal.any === 'function' && signal !== undefined) {
        signal = AbortSignal.any([signal, externalSignal])
      }
      try {
        ...
        if (!res.ok) {
          if (res.status === 401 || res.status === 403) {
            resetToken()
            throw new Error('鉴权失败(HTTP ' + res.status + '):请刷新页面重新获取令牌')
          }
          let detail = ''
          try { detail = await res.text() } catch (err) { /* 忽略 */ }
          // Host 现在回稳定 code + 白名单文案;这里优先展示 code 便于排障
          let code = ''
          try { code = JSON.parse(detail).code || '' } catch (err) { /* 非 JSON */ }
          throw new Error('增强服务返回 ' + res.status + (code.length > 0 ? ' [' + code + ']' : ''))
        }
    ...
    function EnhanceButton(props) {
      ...
      // v0.7.0:**删除已死的客户端历史链**。旧版依次尝试 useSession →
      // useConversation.nodes → props.session 取消息历史,但 0.1.6-alpha.2 下:
      //   · SessionSnapshot 没有 nodes(只有 running/pendingSubmissions/…)
      //   · ConversationSnapshot 只有 {views, activeTargets}
      // 三条分支全部取不到历史,extractHistory 恒返回 [] —— 多轮增强一直是
      // **静默单轮**。历史改由 Host 用 sessions.deriveMessages() 派生(权威来源:
      // 框架喂给 LLM 的同一份派生历史,感知 compaction),客户端只需发 sessionId。
      //
      // 备查(一条曾被误判为「不存在」的路,记录以免后人重走弯路):框架里
      // **确实存在** props.useTrajectory → TrajectorySnapshot.eventNodes:
      // ConversationNode[],且 UserMessageNode.content / AssistantMessageNode.blocks
      // 与旧 extractHistory 期望的形状一致 —— 所以「客户端取历史」并非不可能。
      // 之所以不用:eventNodes 是**事件窗口**视图(还带 eventLocations/partial),
      // 不是权威派生历史,长会话下会缺头部;而 sessions.deriveMessages() 已经是
      // 框架自己喂模型的那一份,且能在 host 侧用纯单测覆盖。
      ...
      // ==================== 引用保护闸门(P0 修复,v0.7.0) ====================
      // 旧版写的是 `newFramework && refs.length > 0` —— 而 newFramework 是**版本嗅探**:
      //   newFramework = (typeof useSession !== 'function') && (typeof useConversation === 'function')
      // 0.1.6-alpha.2 **同时**提供 useSession 与 useConversation → newFramework 恒为 false
      // → 闸门**一直是关着的**。于是含引用的草稿照样送去增强,而回填走 setDraft(纯文本)
      // 会把 Lexical 的引用 chip 重建为样式化文本(公开 API 无引用重插动词)。
      //
      // 修法:版本嗅探换成**事实判定** —— 只看 occurrence 表里是否真有引用。
      // 失败方向是 fail-closed:宁可不增强,也不冒引用状态丢失的风险。
      const refGuardBlocked = refs !== null && refs.length > 0
      ...
      // ==================== 在途请求的取消(v0.7.0) ====================
      // 旧版只在 150s 超时里 abort,**组件卸载 / 会话切换都不取消** —— 于是切了会话
      // 之后旧请求仍会跑完,并把增强文 setDraft 进**新会话的草稿**。这里持有当前
      // 在途请求的 controller,并在卸载时 abort;会话切换时再主动放弃上一个结果。
      const inflightRef = React.useRef(null)
      React.useEffect(() => () => {
        const c = inflightRef.current
        inflightRef.current = null
        if (c !== null && c !== undefined) { try { c.abort() } catch (err) { /* 已结算 */ } }
      }, [])
      // 会话切换(或首次拿到 sessionId):上一会话的在途增强已无意义,立即取消
      const sessionKeyRef = React.useRef(sessionId)
      React.useEffect(() => {
      ...
      const handleClick = async () => {
        ...
        inflightRef.current = abortCtl
        try {
          const result = await enhanceRpc(textPart, sessionId, abortCtl !== null ? abortCtl.signal : undefined)
          // 已被取消(切会话 / 卸载):结果作废,绝不写进别的会话的草稿
          if (abortCtl !== null && abortCtl.signal.aborted === true) return
          ...
          // 主动取消(切会话 / 卸载)不该报「超时」吓用户
          if (!(abortCtl !== null && abortCtl.signal.aborted === true && isAbort)) {
            setError(isAbort ? '增强超时,请重试' : (err instanceof Error ? err.message : String(err)))
          }
        } finally {
          if (inflightRef.current === abortCtl) inflightRef.current = null
          setBusy(false)
        }
      }
    ...
    // 设置服务解析(v0.7.2):
    //   · rc.2+ :框架客户端不再提供 settingsScope,改为 configForms.get(条目 id)
    //             (dsh-client-ui-settings 提供);接口同形:getSnapshot() / set(field, value) / subscribe()。
    //   · 旧版  :settingsScope.bind({ namespace })。
    // 两者都不可用时不挂载设置栏目 —— 魔棒按钮完全不受影响。
    function resolveSettingsScope(ctx) {
      try {
        const forms = ctx.get('configForms')
        if (forms !== undefined && typeof forms.get === 'function') {
          const form = forms.get(SETTINGS_NS)
          if (form !== undefined && typeof form.getSnapshot === 'function' && typeof form.set === 'function') return form
        }
      } catch (err) { /* 继续尝试旧服务 */ }
      try {
        const legacy = ctx.get('settingsScope')
        if (legacy !== undefined && typeof legacy.bind === 'function') {
          const bound = legacy.bind({ namespace: SETTINGS_NS })
          if (bound !== undefined && typeof bound.getSnapshot === 'function' && typeof bound.set === 'function') return bound
        }
      } catch (err) { /* 无可用设置服务 */ }
      return undefined
    }

    // 惰性挂载:设置服务与槽位都可能晚于本插件就绪,重试若干次后放弃,
    // 魔棒按钮完全不受影响。入口为设置后台左侧一级栏目槽位 settings.section
    // (与「提示词库」同款,官方插件设置示例槽位);页面单选模式、即选即存。
    function mountSettingsCard(ctx, slots) {
      if (resolveSettingsScope(ctx) === undefined) return false
      let mounted = false
      let disposed = false
      let scope = undefined
      ...
      const tryMount = () => {
        ...
        try {
          const bound = resolveSettingsScope(ctx)
        ...
        // v0.7.0:同样需要进程令牌(与 enhance 一致的第二道闸)。
        if (typeof fetch === 'function' && typeof AbortController === 'function') {
          const controller = new AbortController()
          const timer = setTimeout(() => controller.abort(), 8000)
          fetchToken()
            .then((token) => fetch('/prompt-enhance/api/modes', {
              headers: { Accept: 'application/json', 'X-Prompt-Enhance-Token': token },
              credentials: 'same-origin',
      ...
      tryMount()
      if (!mounted) {
        // 加载顺序竞态(设置区插件尚未声明槽位/服务)下重试。
        // v0.7.0:删除 ctx.on('service', …) —— cordis 4.x **不存在**名为 'service'
        // 的事件(真实名是 'internal/service'),ctx.on() 对未知事件静默容忍,
        // 所以那个兜底从未生效过(host 半 v0.6.0 已删同款,客户端半当时漏改)。
        // 改用声明式等待:slots.inject 的声明一出现即同步回调,这才是正解。
        ...
        try {
          if (typeof ctx.inject === 'function') {
            // v0.7.2:只等 slots —— rc.2 没有 settingsScope 这个服务名,写进 inject 会永不回调;
            // 设置服务本身由上面的定时重试 + tryMount 内的 resolveSettingsScope 兜住。
            ctx.inject(['slots'], () => { if (!mounted && !disposed) tryMount() })
          }
        } catch (err) { /* ignore */ }
      }
      ctx.effect(() => () => {
    ...
    function mountSettingsNavIcon(ctx) {
      ...
      let timer = undefined
      let bodyObserver = undefined
      let panelObserver = undefined
      let closePoll = undefined
      ...
      // ==================== 作用域收敛(v0.7.0) ====================
      // 旧版常驻观察 document.body + subtree:true —— 流式输出/长会话列表这类
      // 高频 DOM 变更都会排队触发 120ms 去抖全扫,属持续 CPU 开销。
      // 现在分两层,只在设置面板**真的打开**时才做细粒度观察:
      //   · body 观察器:childList 但 **不递归**(面板是 body 的直接子节点),
      //     只用来发现「面板出现了」,平时几乎零成本;
      //   · 面板观察器:面板打开期间观察面板 subtree(框架重渲染 svg 时自愈);
      //   · 关闭轮询:面板消失即断开面板观察器与轮询。桌面设置面板的开关是
      //     用户级低频事件,500ms 轮询的代价远低于全站级重扫。
      const stopPanelWatch = () => {
        if (panelObserver !== undefined) { try { panelObserver.disconnect() } catch (err) { /* ignore */ } }
        panelObserver = undefined
        if (closePoll !== undefined) { clearInterval(closePoll); closePoll = undefined }
      }
      const startPanelWatch = (panel) => {
        if (panelObserver !== undefined) return
        try {
          panelObserver = new MutationObserver(schedule)
          panelObserver.observe(panel, { childList: true, subtree: true })
        } catch (err) { panelObserver = undefined }
        closePoll = setInterval(() => {
          if (document.querySelector('div[role="dialog"]') === null) stopPanelWatch()
        }, 500)
        schedule()
      }
      const onBodyMutation = () => {
        const panel = document.querySelector('div[role="dialog"]')
        if (panel === null) { stopPanelWatch(); return }
        startPanelWatch(panel)
      }
      try {
      ...
      ctx.effect(() => () => {
        ...
        stopPanelWatch()
        if (bodyObserver !== undefined) { try { bodyObserver.disconnect() } catch (err) { /* ignore */ } }
      })
      return true
    }
    ...
    function apply(ctx) {
      ...
      const switchMode = async (modeId, baseText, sessionId) => {
        try {
          // v0.7.2:rc.2 用 configForms,旧框架用 settingsScope —— 统一走 resolveSettingsScope
        ...
        try {
          // 历史由 Host 派生(sessions.deriveMessages()),客户端只发 sessionId
