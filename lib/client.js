  ...
  factory: (require) => {
    ...
    function EnhanceButton(props) {
      ...
      const onSugSwitch = async () => {
        ...
        setError('')
        try {
          const enhanced = await props.switchMode(suggestSnapNow.suggestion, suggestSnapNow.baseText)
    ...
    function apply(ctx) {
      ...
      const switchMode = async (modeId, baseText) => {
        try {
          const settingsScope = ctx.get('settingsScope')
        ...
        try {
          const result = await enhanceRpc(baseText, [])
          if (result !== null && result !== undefined && result.ok === true && typeof result.enhanced === 'string' && result.enhanced.trim().length > 0) {
